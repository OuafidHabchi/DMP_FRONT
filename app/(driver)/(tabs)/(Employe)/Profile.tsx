...
import { StyleSheet, Text, View, TouchableOpacity, Modal, TextInput, Alert, ScrollView, Platform } from 'react-native';
...
export default function Profile() {
  ...
  const handleSave = async () => {
    ...
    const updatedUserData = {
      ...
      language: updatedUser.language, // ✅ Ajout du champ language
    };
    ...
    <View style={styles.container}>
      ...
      <ScrollView>
        <View style={styles.infoContainer}>
          {['name', 'familyName', 'tel', 'email', 'language'].map((field) => (
            <View style={styles.card} key={field}>
              <Text style={styles.label}>
                {user.language === 'English'
                  ? field.charAt(0).toUpperCase() + field.slice(1) // Met la première lettre en majuscule en anglais
                  : field === 'language'
                    ? 'Langue'
                    : field}
              </Text>
      ...
      <Modal animationType="fade" transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalContainer}>
          <View style={styles.modalView}>
            ...
            <PickerModal
              title={user.language === 'English' ? 'Select a Language' : 'Sélectionnez une langue'}
              options={[
                { label: 'English', value: 'English' },
                { label: 'Français', value: 'Français' },
              ]}
              selectedValue={updatedUser.language}
              onValueChange={(value) => setUpdatedUser((prev) => ({ ...prev, language: value }))}
            />
...
const styles = StyleSheet.create({
  ...
  showPasswordButton: { marginLeft: -60, marginTop: -10 },
