...
import { FlatList,StyleSheet, Text, View, TouchableOpacity, Modal, ActivityIndicator, Animated, Linking, ScrollView } from 'react-native';
    ...
    const handleProcedureClick = async (procedure: Procedure) => {
        try {
            if (!procedure.seen.includes(user?._id ?? "")) {
                ...
                setProcedures(prev =>
                    prev.map(proc =>
                        proc._id === procedure._id
                            ? { ...proc, seen: [...new Set([...proc.seen, user?._id ?? ""])] } // ✅ Filtrage des valeurs `undefined`
                            : proc
                    )
                );
            }
