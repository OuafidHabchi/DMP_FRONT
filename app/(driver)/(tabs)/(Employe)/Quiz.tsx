    ...
    try {
      ...
      Alert.alert(
        ...
        user?.language === 'English'
          ? 'Unable to fetch your assigned quizzes.'
          : 'Impossible de récupérer vos quiz assignés.'
      );
    }
  };
  ...
  const handleSelectQuiz = async (quiz: QuizAssignment) => {
    ...
    try {
      ...
      Alert.alert(
        user?.language === 'English' ? 'Error' : 'Erreur',
        user?.language === 'English'
          ? 'Unable to load the questions for this quiz.'
          : 'Impossible de charger les questions de ce quiz.'
      );
    }
  };
  ...
  const submitResults = async (finalAnswers: Answer[], calculatedScore: number) => {
    try {
      ...
      if (response.status === 200) {
        Alert.alert(
          user?.language === 'English' ? '✅ Quiz submitted successfully!' : '✅ Quiz soumis avec succès !'
        );
      } else {
        Alert.alert(
          user?.language === 'English' ? '❌ Error during submission.' : '❌ Erreur lors de la soumission.'
        );
      }
    } catch (error) {
      Alert.alert(
        user?.language === 'English' ? '⚠️ Error' : '⚠️ Erreur',
        user?.language === 'English' ? 'Unable to submit the results.' : "Impossible de soumettre les résultats."
      );
    }
  };
            ...
            >
              ...
              <View style={styles.resultDetails}>
                <Text style={styles.resultText}>
                  {user?.language === 'English' ? 'Your answer: ' : 'Votre réponse : '}
                  {item.selectedOption}
                </Text>

                <Text style={styles.correctAnswer}>
                  {user?.language === 'English' ? 'Correct answer: ' : 'Réponse correcte : '}
                  {item.correctAnswer}
                </Text>
          ...
          <View style={styles.modalContainer}>
            ...
            <ScrollView>
              <Text style={styles.instructionsTitle}>
                {user?.language === 'English' ? 'Instructions' : 'Instructions'}
              </Text>

              <Text style={styles.warningText}>
                {user?.language === 'English'
                  ? '⚠️ You can take the test only once, so read the questions carefully (you need at least 80% to pass).'
                  : '⚠️ Vous ne pouvez passer le test qu\'une seule fois, alors lisez les questions attentivement (vous devez obtenir au moins 80% pour réussir).'}
                {"\n"}{"\n"}
              </Text>
              ...
              >
                <Text style={styles.startButtonText}>
                  {user?.language === 'English' ? "I'm ready for the test" : 'Je suis prêt pour le test'}
                </Text>
              </TouchableOpacity>
      ...
      {!isReady ? (
            ...
            >
              ...
              <Text style={styles.quizStatus}>
                {item.status === 'completed'
                  ? `${user?.language === 'English' ? '✅ Completed - Score: ' : '✅ Terminé - Score: '}${item.score ?? 0}%`
        ...
        <View style={styles.questionBox}>
          {quizQuestions.length > 0 && currentQuestionIndex >= 0 && currentQuestionIndex < quizQuestions.length ? (
            ...
            <Text style={styles.questionText}>
              {user?.language === 'English' ? 'Loading questions or no questions available...' : 'Chargement des questions ou aucune question disponible...'}
            </Text>
          )}
