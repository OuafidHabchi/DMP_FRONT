...
const EmployeeWarnings = () => {
  ...
  const fetchWarningDetails = async (warningId: string) => {
    try {
      ...
      // Vérifier si l'image est bien définie et complète
      if (warningData.photo) {
        if (!warningData.photo) {
          warningData.photo = `${AppURL}/${warningData.photo}`; // Ajoute AppURL si nécessaire
        }
      } else {
        warningData.photo = null; // Évite les erreurs sur `ImageViewer`
      }

      setSelectedWarning(warningData); // Met à jour l'état
    } catch (error) {
      Alert.alert(
  ...
  const markAsRead = async (warningId: string) => {
    try {
      ...
      Alert.alert(
        user?.language === 'English' ? 'Error' : 'Erreur',
        user?.language === 'English'
          ? 'Failed to mark the warning as read.'
          : 'Échec de la mise à jour de l\'avertissement comme lu.'
      );
    }
  };
                      ...
                      <Image
                        source={{
                          uri: selectedWarning?.photo?.startsWith('http')
                            ? selectedWarning.photo // Si l'URL est déjà complète
