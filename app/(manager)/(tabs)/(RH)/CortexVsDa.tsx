...
import { StyleSheet, Text, View, TouchableOpacity, ActivityIndicator, Alert, Platform, FlatList, Modal, Dimensions, TextInput, Pressable } from "react-native";
...
import CortexWarning from "@/components/src/CortexWarning";
...
const CortexVsDa: React.FC = () => {
  ...
  const [isCortexWarningVisible, setIsCortexWarningVisible] = useState(false);
  const [selectedEmployee, setSelectedEmployee] = useState<Employee | null>(null);
  const [selectedEmployeeImage, setSelectedEmployeeImage] = useState<string | null>(null);



  const filteredTimeCards = timeCards.filter((item) => {
        ...
        {Platform.OS === "web" && (
          <Pressable
            ...
            onPress={() => {
              ...
              const timeCard = timeCards.find(tc => tc.employeeId === employee._id);
              setSelectedEmployeeImage(timeCard?.image ? `${AppURL}/uploads-timecard${timeCard.image}` : null);
              setIsCortexWarningVisible(true);
            }}
          >
  ...
  return (
    <View style={styles.container}>
      ...
      {isCortexWarningVisible && selectedEmployee && (
        <CortexWarning
          isVisible={isCortexWarningVisible}
          onClose={() => setIsCortexWarningVisible(false)}
          employee={selectedEmployee}
          employeeImage={selectedEmployeeImage || ""} // ✅ Assurer qu'on passe une string
          dsp_code={user?.dsp_code || ""} // Fournir une valeur par défaut
        />
      )}
...
const styles = StyleSheet.create({
  warningButton: {
    backgroundColor: "red", // Orange pour avertissement
    padding: 4, // Réduction de la taille du padding
    borderRadius: 30, // Bouton circulaire
    justifyContent: "center",
    alignItems: "center",
    width: 20, // Bouton plus petit
    height: 20, // Bouton plus petit
    shadowColor: "#000",
