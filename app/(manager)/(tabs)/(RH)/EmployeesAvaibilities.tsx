...
const EmployeesAvaibilities = () => {
  ...
  const openPredictionModal = () => {
    setIsLoadingPrediction(true); // Active le chargement
    ...
    setIsLoadingPrediction(false); // Désactive le chargement
    ...
    setDisponibilities((prev) =>
      prev.map((dispo) => {
        if (dispo.employeeId === employeeId && new Date(dispo.selectedDay).toDateString() === date.toDateString()) {
          ...
          setHasUnsavedChanges(true);
          // Ajouter la condition pour vérifier si la plateforme est web
          if (Platform.OS === 'web') {
        ...
        <>
          <View style={styles.header}>
            ...
            <View style={[styles.counterContainer, { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }]}>
              ...
              <TouchableOpacity style={[styles.predictionModalSubmitButton, { flex: Platform.OS === 'web' ? 1 : 0 }]} onPress={openPredictionModal}>
              ...
              {hasUnsavedChanges && Platform.OS === 'web' ? (
                <View style={[styles.unsavedChangesContainer, { flex: Platform.OS === 'web' ? 1 : 0, alignItems: 'center', justifyContent: 'center' }]}>
