    ...
    if (event) {
      if (user && (event.invitedGuests.includes(user._id) || event.createdBy === user._id)) {
        ...
        if (Platform.OS === 'web') {
          window.alert(
            user?.language === 'English'
              ? 'You cannot view the details of this event'
              : 'Vous ne pouvez pas voir les détails de cet événement'
          );
        } else {
          Alert.alert(
    ...
    try {
      ...
      if (!eventData.title || !eventData.date || !eventData.heur || !eventData.createdBy || !eventData.duration || !eventData.Link) {
        if (Platform.OS === 'web') {
          window.alert(
            user?.language === 'English'
              ? 'The fields title, date, time, Link are required.'
              : 'Les champs title, date, heure, Link sont requis.'
          );
        } else {
          Alert.alert(
            user?.language === 'English' ? 'Error' : 'Erreur',
      ...
      if (currentEvent) {
        if (user?._id === currentEvent.createdBy) {
          ...
          if (Platform.OS === 'web') {
            window.alert(
              user?.language === 'English'
                ? 'You can only view the details of this event.'
                : 'Vous ne pouvez que visualiser les détails de cet événement.'
            );
          } else {
            Alert.alert(
              user?.language === 'English' ? 'Access Denied' : 'Accès refusé',
