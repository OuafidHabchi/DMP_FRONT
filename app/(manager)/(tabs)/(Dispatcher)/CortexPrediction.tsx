...
const CortexPrediction: React.FC = () => {
        ...
        if (timeCard?.image) {
            try {
                ...
                if (response.ok) {
                    ...
                    if (Platform.OS === 'web') {
                        window.alert(user?.language === "English" ? "Image not found" : "Image introuvable");
                    } else {
                        Alert.alert(user?.language === "English" ? "Image not found" : "Image introuvable");
                    }
                ...
                if (Platform.OS === 'web') {
                    window.alert(user?.language === "English" ? "Failed to load image" : "Échec du chargement de l'image");
                } else {
                    Alert.alert(user?.language === "English" ? "Failed to load image" : "Échec du chargement de l'image");
                }
    ...
    const savePhoto = async () => {
        if (!selectedEmployeeId || !selectedImage) {
            if (Platform.OS === 'web') {
                window.alert(user?.language === "English" ? "No employee or image selected" : "Aucun employé ou image sélectionné");
            } else {
                Alert.alert(user?.language === "English" ? "No employee or image selected" : "Aucun employé ou image sélectionné");
            }
        ...
        if (!timeCard) {
            if (Platform.OS === 'web') {
                window.alert(user?.language === "English" ? "No time card found for this employee." : "Aucune fiche de temps trouvée pour cet employé.");
            } else {
                Alert.alert(user?.language === "English" ? "No time card found for this employee." : "Aucune fiche de temps trouvée pour cet employé.");
            }
        ...
        try {
            ...
            if (response.status === 200) {
                if (Platform.OS === 'web') {
                    window.alert(user?.language === 'English' ? 'Photo saved successfully!' : 'Photo enregistrée avec succès!');
                } else {
                    Alert.alert(
                ...
                if (Platform.OS === 'web') {
                    window.alert(user?.language === 'English' ? 'Failed to save the photo.' : 'Échec de l’enregistrement de la photo.');
                } else {
                    Alert.alert(
            ...
            if (Platform.OS === 'web') {
                window.alert(user?.language === 'English' ? 'An unexpected error occurred.' : 'Une erreur inattendue est survenue.');
            } else {
                Alert.alert(
                    user?.language === 'English' ? 'Error' : 'Erreur',
