...
export default function Accueil() {
  ...
  // Vérifier le chargement de l'image dès que selectedNote change
  useEffect(() => {
    if (selectedNote?.photo) {
      setIsImageFullyLoaded(false);
      handleImageLoad(`${AppURL.replace(/\/$/, '')}/${selectedNote.photo.replace(/^\//, '')}`);
    }
  }, [selectedNote?.photo]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal visible={isModalVisible} transparent animationType="none">
              ...
              <>
                ...
                {selectedNote?.photo && (
                  <View style={styles.imagePreviewContainer}>
                    {!isImageFullyLoaded ? (
                      <ActivityIndicator size="large" color="#fff" style={styles.loadingIndicatorSpinner} />
                    ) : (
                      <Pressable onPress={() => setIsModalVisibleImage(true)}>
                ...
                <Pressable style={styles.enhancedCloseButton} onPress={() => setIsModalVisible(false)}>
        ...
        {selectedNote?.photo && isImageFullyLoaded && (
          <Modal visible={isModalVisibleImage} transparent animationType="fade">
            <ImageViewer
              imageUrls={[{
                url: selectedNote?.photo
                  ? `${AppURL.replace(/\/$/, '')}/${selectedNote.photo.replace(/^\//, '')}`
                  : ''
              }]}
              enableSwipeDown={true}
              onSwipeDown={() => setIsModalVisibleImage(false)}
              renderHeader={() => (
                <Pressable style={styles.closeButtonImage} onPress={() => setIsModalVisibleImage(false)}>
